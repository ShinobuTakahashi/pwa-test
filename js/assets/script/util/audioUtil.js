...
(function (audioUtil) {
    ...
    function addAssetIdsFromSoundInfoMap(_map, _assetIds) {
        ...
        Object.keys(_map).forEach((i) => {
            ...
            Object.keys(info).forEach((j) => {
                if (j === "isServer")
                    return;
                const assetId = info[j];
                if (!assetId)
                    return;
    ...
    function inUse(_soundId, opt_assets) {
        ...
        const asset = opt_assets[_soundId];
        if (!asset) {
    ...
    function getPlayingAudioInfo(_asset) {
        const list = playingAudioInfoList.filter((v) => { return v.audioAsset === _asset; });
        if (list.length > 0) {
            return list[0];
        }
        return null;
