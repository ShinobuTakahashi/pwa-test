window.gLocalAssetContainer["popScore"] = function(g) { (function(exports, require, module, __filename, __dirname) {
"use strict";
...
class PopScore {
     ...
     */
    constructor(_scene, _parent, _fontPlus, _fontMinus, _asaPjName, _plusAnimName, _minusAnimName, _actorPosOffsetY, _labelPosOffsetY, _pivot, _digit) {
        this.scene = _scene;
        this.layer = _parent;
     ...
     */
    createPopScore(_pos, _value) {
        let popAnime = this.plusAnimName;
        let font = this.fontPlus; // プラスフォント
        let sign = "+"; // 符号
        if (_value < 0) { // マイナスの場合
        ...
        // ポイント演出用ダミーアニメ
        const actorPopPoint = new asaEx_1.asaEx.Actor(this.scene, this.asaPjName);
        ...
        const label = entityUtil_1.entityUtil.createNumLabel(this.scene, font, this.digit + 1);
        entityUtil_1.entityUtil.moveNumLabelTo(// appendした箇所からの相対座標
        label, 0 + label.width, 0 + this.labelPosOffsetY - (label.height / 2));
        entityUtil_1.entityUtil.setLabelText(label, sign + String(_value)); // 符号つきテキスト
        // アタッチメント
        const attachment = new asaEx_1.asaEx.EntityAttachment(label);
        actorPopPoint.attach(attachment, this.pivot); // ダミーアニメにアタッチ
        actorPopPoint.update.handle(actorPopPoint, () => {
