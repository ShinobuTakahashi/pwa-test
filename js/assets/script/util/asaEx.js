...
(function (asaEx) {
    ...
    class ResourceManager {
         ...
         */
        static addAsaAssetIds(_pjNames, _assets, _assetIds) {
            const ids = ResourceManager.getAssetIds(_pjNames, _assets);
         ...
         */
        static getAssetIds(_pjNames, _assets) {
            let res = [];
            const iEnd = _pjNames.length;
            for (let i = 0; i < iEnd; ++i) {
                ...
                const pjJson = _assets[pjName].data;
                const pjData = JSON.parse(pjJson);
                const fileNames = [pjName].concat(pjData.contents.boneSetFileNames, pjData.contents.skinFileNames, pjData.contents.animationFileNames);
         ...
         * @return スキン名の配列
         */
        static getSkinNames(_resource) {
            const res = [];
            const iEnd = _resource.skins.length;
            for (let i = 0; i < iEnd; ++i) {
         ...
         * @return ボーン名
         */
        static getBoneName(_pjName, _animName) {
            const tableMap = g.game.vars.asaAnimBoneTableMap;
            const table = tableMap[_pjName];
         ...
         * @param _pjName asapjのアセット名
         */
        static removeLoadedResource(pjName) {
         ...
         * すべて削除する
         */
        static removeAllLoadedResource() {
            if (g.game.vars.asaAnimBoneTableMap) {
    ...
    class Actor extends asa.Actor {
         ...
         * 省略時はtrue。
         */
        constructor(_scene, _pjName, opt_animName, opt_loopFlag = true) {
            if (!g.game.vars.asaAnimBoneTableMap) {
            ...
            const resource = ResourceManager.getResource(_scene, _pjName);
            if (!opt_animName) {
         ...
         * @override
         */
        play(_animName, _startFrame, _loopFlag, _playSpeed, opt_noCalcFlag = false) {
            const currBnName = this.getBoneName();
            const nextBnName = this.getBoneName(_animName);
         ...
         * @return ボーンの座標
         */
        getBonePosition(_boneName, opt_matrix) {
            const inScene = this.getBonePositionInScene(_boneName, opt_matrix);
            const rootMatrix = this.getMatrix()._matrix;
         ...
         * @param _boneName ボーン名
         */
        changeBone(_boneName) {
            // vvv akashic-animation/lib/Actor.js より引用
            // skeleton
            const boneSet = this.resource.getBoneSetByName(_boneName);
    ...
    class EntityAttachment extends asa.Attachment {
        ...
        render(_renderer) {
            if (this.cancelParentSR) {
                ...
                const m0 = [1, 0, 0, 1, 0, 0];
                const m1 = this.posture.m._matrix;
                const m2 = this.matrix._matrix;
                // m0 = m1 * m2
     ...
     */
    class ActorAttachment extends EntityAttachment {
        /**
         * ActorAttachmentコンストラクタ
         * @param _actor アタッチするActorのインスタンス
         * @param opt_matrix (optional)アタッチ先のボーンに対しての位置や向きを変えるための変換行列
         */
        constructor(_actor, opt_matrix) {
            if (opt_matrix) {
    ...
    function makeAnimBoneTable_(_combinationInfos) {
        const res = {};
        const iEnd = _combinationInfos.length;
        for (let i = 0; i < iEnd; ++i) {
    ...
    function loadAnimBoneTable_(_pjName, _assets) {
        ...
        let res = null;
        const pjJson = _assets[_pjName].data;
        const pjData = JSON.parse(pjJson);
...
function setupCollider_(_bones, _actor) {
    _bones.forEach((_bone) => {
        if (!_bone.colliderInfos) {
            return;
        }
        _bone.colliderInfos.forEach((_info) => {
            let collider;
