...
(function (gameUtil) {
    ...
    function addOneGlyphMapFromFrame(_oneChar, _json, _frameName, _map) {
        const frame = _json.frames[_frameName].frame;
    ...
    function createNumFontWithAssetInfo(_info, opt_assets) {
        ...
        const glyphMap = gameUtil.makeGlyphMapFromFrames(exports.CHAR_CODE_0, exports.CHAR_CODE_10, frameMap, _info.numFrames);
        ...
        let missingGlyph;
        if (_info.missing) {
            const frame = frameMap.frames[_info.missing].frame;
            missingGlyph = {
                x: frame.x,
                y: frame.y,
                width: frame.w,
    ...
    function updateGameStateScore(_score) {
        const gameState = g.game.vars.gameState;
    ...
    function getGameScore() {
        ...
        let score = g.game.vars.scenedata.gameScore;
        if (!score) {
            score = 0;
        }
        return score;
